...
// EXTMEM String inputstream = "";
// EXTMEM String qstream = "";
...
//		Serial.print(c);
	...
	if (bitidx > 0)
	{
		//	int f = inputstream.indexOf("`");
		//	int e = inputstream.lastIndexOf("`");
		//		Serial.println(String(inputstream.length()) + " " + inputstream.substring(0, 50) + "..." + inputstream.substring(inputstream.length() - 50));
		//		Serial.println(String(f) + " ___ "+ String(e));
		bitstream[bitidx++] = 0;
		bitstream[colidx] = 0;
#if0
		Serial.print(bitidx);
...
{
	//	Serial.println(String(inputstream.length())+" "+inputstream);
	//	signed char cc = SplitS(inputstream, '\n', (String *)commands, 15);
	//	if (cc - 1 == 0)
	//	{
	//		inputstream = ""; // delete all
	//		return;
	//	}
	//	Serial.println("input: " + inputstream.replace("\n", "") + " ->" + inputstream.substring(0, 6));
	//	bool ismousedown = false;
	//		Serial.print(String(ci)+" "+commands[ci]);
	int lc = colidx + 1;
	String para[20];
	for (int i = 0; i < commidx; i++)
	{
